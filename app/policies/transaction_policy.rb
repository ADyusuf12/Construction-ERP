class TransactionPolicy < ApplicationPolicy
  ...
  def index?
    ...
    user.role_accountant? || user.role_cto? || user.role_manager? ||
  ...
  def create?
    # CEO/Admin/Accountant/CTO/Manager can create transactions
    user.role_ceo? || user.role_admin? || user.role_accountant? || user.role_cto? || user.role_manager?
  end
  ...
  class Scope < Scope
    def resolve
      if user.role_ceo? || user.role_admin? || user.role_accountant? || user.role_cto? || user.role_manager?
        scope.all
      else
